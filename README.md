...
Ctrl+P 打开命令，输入“看板MOC”，可以看到一共有 7 条命令：
...
点击确定，新建完成后关闭面板。回到🌏资源MOC文档，可以看到现在新增了一个名称为“测试”的卡片：
![image-20211218140224656](attachments/image-20211218140224656.png)

点击“测试”跳转至该页面，这个页面是调用事先创建的模板文件创建的：![image-20211218140331403](attachments/image-20211218140331403.png)
...
> 有了这个命令就可以把一个项目文件夹移动到资源文件夹，然后点击更新索引

#### 2.3、修改资源“测试”的名称为：测试2

运行命令：`看板MOC: 修改资源名称`，修改资源“测试”的名称为：测试2
...
运行命令：`看板MOC: 删除资源`，输入资源名称“测试2”，然后手动输入：“确认删除”，点击确定删除文件：
![image-20211218143049716](attachments/image-20211218143049716.png)

可以看到删除是直接删除这个资源（或项目）文件夹。电脑端删除的文件会放在回收站（手机端删了我也不知道去哪😵‍💫）

> 删除资源（或项目）时目前不会自动删除MOC中对该资源（或项目）索引的卡片，需要手动删除。
